 ...
 * Zero-dependency decoder for Exchange ActiveSync binary payloads.
 * ============================================================================
 */
class MyCloudWBXML {
    private $stream;
    ...
    // Core EAS Code Pages required for Mail, Folders, and Provisioning
    private $codePages = [
        0 => [ 5=>'Sync', 6=>'Responses', 7=>'Add', 8=>'Change', 9=>'Delete', 10=>'Fetch', 11=>'SyncKey', 12=>'ClientId', 13=>'ServerId', 14=>'Status', 15=>'Collection', 16=>'Class', 18=>'CollectionId', 19=>'GetChanges', 22=>'WindowSize', 28=>'Commands', 29=>'Options', 30=>'FilterType', 36=>'Conflict', 38=>'ApplicationData' ],
        ...
        14=> [ 5=>'Provision', 6=>'Policies', 7=>'Policy', 8=>'PolicyType', 9=>'PolicyKey', 10=>'Data', 11=>'Status', 12=>'RemoteWipe' ],
        15=> [ 5=>'To', 6=>'Cc', 7=>'From', 8=>'Subject', 9=>'ReplyTo', 10=>'DateReceived', 11=>'DisplayTo', 12=>'ThreadTopic', 13=>'Importance', 14=>'Read', 15=>'MessageClass', 18=>'Flag', 19=>'Status' ],
        17=> [ 5=>'BodyPreference', 6=>'Type', 7=>'TruncationSize', 8=>'AllOrNone', 10=>'Body', 11=>'Data', 12=>'EstimatedDataSize', 14=>'Attachments', 15=>'Attachment', 16=>'DisplayName', 17=>'FileReference', 18=>'Method', 19=>'ContentId' ],
        21=> [ 5=>'ItemOperations', 6=>'Fetch', 7=>'Store', 8=>'Options', 9=>'Range', 10=>'Total', 11=>'Properties', 12=>'Data', 13=>'Status', 14=>'Response' ]
    ];
        ...
        while ($this->pos < strlen($this->stream)) {
            ...
            switch ($token) {
                ...
                case 0xC3: 
                    ...
                    break;
                default:
                    $hasAttributes = ($token & 0x40) > 0;
                    $hasContent = ($token & 0x80) > 0;
                    $tagId = $token & 0x3F;
...
class MyCloudEASClient {
    ...
    private function request($command, $xmlPayload = null, $isBinary = false) {
        ...
        $url = $this->host . "/Microsoft-Server-ActiveSync?Cmd={$command}&User={$this->user}&DeviceId={$this->deviceId}&DeviceType=WindowsOutlook";
        ...
        try {
            return $this->wbxml->decode($respBody);
        } catch (\Exception $e) {
    ...
    private function provisionDevice() {
        $reqXml = '<?xml version="1.0" encoding="utf-8"?><Provision xmlns="Provision"><Policies><Policy><PolicyType>MS-EAS-Provisioning-WBXML</PolicyType></Policy></Policies></Provision>';
    ...
    private function getSyncKey($folderId) {
        ...
        return $this->getNodeValue($initDom, 'SyncKey', '0');
    }
    ...
    private function getFolderSyncKey() {
        $xml = '<?xml version="1.0" encoding="utf-8"?><FolderSync xmlns="FolderHierarchy"><SyncKey>0</SyncKey></FolderSync>';
        $dom = $this->request('FolderSync', $xml);
        return $this->getNodeValue($dom, 'SyncKey', '0');
    }
    ...
    public function getFolders() {
        ...
        $folders = [];
        if (!$dom) return $folders;

        $adds = $this->getNodes($dom, 'Add');
        foreach ($adds as $add) {
            $folders[] = [
    ...
    public function getMessages($folderId, $page = 1, $perPage = 50) {
        ...
        foreach ($changes as $change) {
            ...
            $date = $this->getNodeValue($appData, 'DateReceived');
            ...
            // Parse Flag status (2 = flagged)
            $flagStatus = $this->getNodeValue($this->getNodes($appData, 'Flag')[0] ?? null, 'Status', '0');

            $toRaw = $this->getNodeValue($appData, 'To');
            $ccRaw = $this->getNodeValue($appData, 'Cc');
            $bccRaw = $this->getNodeValue($appData, 'Bcc');
            $replyToRaw = $this->getNodeValue($appData, 'ReplyTo');
...
// --- PGP WKD HASHING HELPERS ---
    ...
    private function sendSmtpMail($acc, $to, $subject, $body, $fromAlias = null, $cc = '', $bcc = '', $attachments = [], $dryRunMimeOnly = false, $requestReceipt = false) {
        ...
        if (!$dryRunMimeOnly) {
            ...
            $recipients = explode(',', $to . ',' . $cc . ',' . $bcc);
            foreach($recipients as $recipient) {
                $recipient = trim($recipient);
                if (empty($recipient)) continue;
                
                $clean_email = trim($recipient);
                
                // extract <email> safely
                if (preg_match('/<([^<>]+)>/', $clean_email, $m)) {
                    $clean_email = $m[1];
                }
                
                // Strip malicious chars but allow RFC 5322 compliant email characters
                $clean_email = preg_replace('/[^a-zA-Z0-9!#$%&\'*+\-\/=?^_`{|}~.@]/', '', $clean_email);
                
                if (!filter_var($clean_email, FILTER_VALIDATE_EMAIL)) continue;
        ...
        switch ($action) {
                    ...
                    foreach ($resolvedFolders as $idx => $fld) {
                        ...
                        if (($searchFailed || $statusFailed || (empty($uids) && $s_messages > 0)) && $searchQuery === '') {
                            ...
                            foreach (glob($this->cache_dir . "/{$id}_{$safeFld}_*.json.enc") as $file) {
                                ...
                                foreach (array_keys($cData) as $k) if ($k !== '__FOLDER_STATE__') $uids[] = (int)$k;
                            }
                        }
            ...
            case 'email_send':
                ...
                if (empty($_POST['is_draft'])) {
                    ...
                    $payload = [
                        'to'      => $cleanHeader($_POST['to'] ?? ''),
                        'cc'      => $cleanHeader($_POST['cc'] ?? ''),
                        'bcc'     => $cleanHeader($_POST['bcc'] ?? ''),
