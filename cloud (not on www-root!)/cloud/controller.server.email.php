    ...
    private function connectImap($acc, $folder = '', $readonly = false) {
        ...
        while ($attempt < $maxRetries) {
            try {
                ...
                if (stripos($lastErr, 'wrong version number') !== false || stripos($lastErr, 'reset by peer') !== false || stripos($lastErr, 'timeout') !== false) {
                    $attempt++;
                ...
                return [null, null, "IMAP Connection failed: " . $lastErr];
            }
        }
        return [null, null, "IMAP Connection failed after retries: " . $lastErr];
    }
        ...
        switch ($action) {
            ...
            case 'email_dl_pdf':
                ...
                try {
                    ...
                    $pdfBodyLast = isset($_POST['pdf_body_last']) && $_POST['pdf_body_last'] === '1';
                    $pdfAttOrder = json_decode($_POST['pdf_attachment_order'] ?? '[]', true);
                    $pdfExcluded = json_decode($_POST['pdf_excluded_atts'] ?? '[]', true);
                    ...
                    foreach ($filteredAttachments as $att) {
                        ...
                        if ($attExt === 'pdf') {
                            ...
                            if ($qRet === 0) $attachFiles[] = ['name' => $attName, 'path' => $tmpPath];
                            else $processedPdfs[] = ['name' => $attName, 'path' => $tmpPath];
                            ...
                            if (!in_array($realMime, ['image/jpeg', 'image/png', 'image/gif', 'image/webp', 'image/bmp'])) {
                                $attachFiles[] = ['name' => $attName, 'path' => $tmpPath];
                                continue;
                            }
                    ...
                    if (is_array($pdfExcluded) && !empty($pdfExcluded)) {
                        $safeExcluded = array_map(function($n) { return preg_replace('/[^a-zA-Z0-9.\-_ ]/', '_', $n); }, $pdfExcluded);
                        
                        foreach ($processedPdfs as $idx => $pData) {
                            if (in_array($pData['name'], $safeExcluded)) {
                                @unlink($pData['path']);
                                unset($processedPdfs[$idx]);
                            }
                        }
                        $processedPdfs = array_values($processedPdfs);
                        
                        foreach ($attachFiles as $idx => $aData) {
                            if (in_array($aData['name'], $safeExcluded)) {
                                @unlink($aData['path']);
                                unset($attachFiles[$idx]);
                            }
                        }
                        $attachFiles = array_values($attachFiles);
                    }

                    if ($pdfBodyLast) {
                        if (is_array($pdfAttOrder) && !empty($pdfAttOrder)) {
                            foreach ($pdfAttOrder as $orderedName) {
                                $safeOrderedName = preg_replace('/[^a-zA-Z0-9.\-_ ]/', '_', $orderedName);
                                foreach ($processedPdfs as $idx => $pData) {
                                    if ($pData['name'] === $safeOrderedName) {
                                        $mergePdfs[] = $pData['path'];
                                        unset($processedPdfs[$idx]);
                                        break;
                                    }
                                }
                        ...
                        foreach ($processedPdfs as $pData) { $mergePdfs[] = $pData['path']; }
                        $mergePdfs[] = $mainPdf;
                    } else {
                        $mergePdfs[] = $mainPdf;
                        foreach ($processedPdfs as $pData) { $mergePdfs[] = $pData['path']; }
