...
window._emailShowPopup = function(e, name, email, extraB64) {
    if (window._emailPopupRaf) cancelAnimationFrame(window._emailPopupRaf);
    if (window._emailPopupHideTimer) clearTimeout(window._emailPopupHideTimer);
    ...
    window._emailPopupRaf = requestAnimationFrame(() => {
...
window._emailHidePopup = function() {
    ...
    if (popup) { 
        ...
        if (window._emailPopupHideTimer) clearTimeout(window._emailPopupHideTimer);
        window._emailPopupHideTimer = setTimeout(() => { if (popup.style.opacity === '0') popup.style.display = 'none'; }, 200); 
    }
};
...
window.myCloudEmailFetchFolders = function(silent = false, specificAccId = null) {
    ...
    if (targetAcc === 'smartbox') {
        ...
        // Instantly render inbox from cache
        if (!silent && !specificAccId) myCloudEmailFetchMessages('SMARTBOX');

        // Delay the heavy IMAP folder polling to prioritize message loading
        setTimeout(() => {
            window._emailExecuteSyncTasks(folderTasks).then(() => {
                myCloudEmailRenderTree();
            });
        }, 800);
    ...
    return new Promise((resolve) => {
        const executeFolderFetch = () => {
            ...
            fetch('', { method: 'POST', body: fd }).then(myCloudCheckResponse).then(res => {
                if (myCloudState.interface !== 'email') return resolve(res);
        ...
        if (!silent && !specificAccId) setTimeout(executeFolderFetch, 800);
        else executeFolderFetch();
    });
  ...
  window._emailListDebounce = requestAnimationFrame(async () => {
    ...
    const appendMsgToDom = (m, isChild = false, parentId = null) => {
        ...
        if (item) {
            ...
            if (currentDomNode && currentDomNode.parentNode === listContent) {
                if (currentDomNode !== item) {
...
window._emailDownloadPdf = function(accId, folder, msgId, forceFallback = false, pdfBodyLast = false, pdfOrder = [], pdfExclude = []) {
	let loadImg = '1';
    ...
    if (pdfBodyLast) {
        payload.pdf_body_last = '1';
        payload.pdf_attachment_order = JSON.stringify(pdfOrder);
		payload.pdf_excluded_atts = JSON.stringify(pdfExclude);
    }
    return window._emailDownloadResource('email_dl_pdf', payload, fallback, 'Generating PDF...', forceFallback);
};
...
window._emailShowReadingContextMenu = function(e, sourceEl, isIframe, accId, folder, msgId) {
    ...
    const closeMenu = (ev) => {
        ...
        if (isIframe && doc) {
            ...
            doc.removeEventListener('touchstart', closeMenu, true);
        }
    };
    ...
    setTimeout(() => {
        ...
        if (isIframe && doc) {
            ...
            doc.addEventListener('touchstart', closeMenu, true);
        }
    }, 10);
...
window._emailShowSaveOptions = async function(msgDataOrAccId, legacyFolder, legacyId) {
    ...
    if (targetKeys.length === 1) {
        if (typeof myCloudShowLoading === 'function') myCloudShowLoading();
        const k = targetKeys[0];
        const parts = k.split('|');
        let cache = myCloudEmailState.bodyCache[k];
        if (cache instanceof Promise) cache = await cache;
        if (!cache) {
            const fd = new URLSearchParams({ myCloud_action: 'email_get_body', myCloud_key: myCloudState.key, myCloud_token: window.myCloudCsrfToken, account_id: parts[0], folder: parts[1], message_id: parts[2] });
            cache = await fetch('', {method:'POST', body:fd}).then(r=>r.json()).catch(()=>null);
            if (cache && cache.status === 'OK') myCloudEmailState.bodyCache[k] = cache;
        }
        if (typeof myCloudHideLoading === 'function') myCloudHideLoading();
        
        if (cache && cache.attachments) {
            pdfAtts = cache.attachments.filter(a => {
                const ext = (a.filename || a.name || '').split('.').pop().toLowerCase();
                return ext === 'pdf' || ['jpg','jpeg','png','gif','webp','bmp'].includes(ext);
            });
        }
    }
    ...
    if (pdfAtts.length > 0) {
        pdfOrderHtml = 
            '<div id="emlPdfOrderWrap" style="margin:0; display:none;">' +
                '<label style="display:flex; align-items:flex-start; gap:8px; cursor:pointer; margin:0;">' +
                    '<input type="checkbox" id="emlPdfBodyLastCb" class="myCloudCheckbox" style="margin-top:2px;">' +
                    '<span style="font-size:13px; color:var(--text-primary); line-height:1.4;">' + (L.pdf_body_last || 'Place PDF attachments first, email body last') + '</span>' +
                '</label>' +
                '<div id="emlPdfReorderList" style="display:none; margin-top:8px; flex-direction:column; gap:4px; max-height:150px; overflow-y:auto; padding-inline-start:24px;"></div>' +
            '</div>';
    }
    ...
    modal.innerHTML = 
        ...
        '</div>' +
        '<div class="myCloudModalBody" style="padding:20px; overflow-y:auto; flex:1; min-height:0;">' +
            ...
            '<div style="display:flex; flex-direction:column; gap:12px; margin-top:15px;">' +
                '<label style="display:flex; align-items:flex-start; gap:8px; cursor:pointer; margin:0;">' +
                    '<input type="checkbox" id="emlSaveDelCb" class="myCloudCheckbox" ' + delAfterChecked + ' style="margin-top:2px;">' +
                    '<span style="font-size:13px; color:var(--text-primary); line-height:1.4;">' + (L.delete_after_save || 'Delete email after saving') + '</span>' +
                '</label>' +
                pdfOrderHtml +
            '</div>' +
         '</div>' +
        '<div style="padding:15px 20px; border-top:1px solid var(--border-default); background:var(--gray-05); flex-shrink:0; border-radius:0 0 6px 6px;">' +
            '<div class="myCloudButtons" style="justify-content:flex-end; margin:0;">' +
                '<button class="owa-btn" onclick="myCloudCloseModal()">' + (L.cancel || 'Cancel') + '</button>' +
                '<button id="btnEmlDoSave" class="owa-btn owa-primary">' + (writeableClouds.length > 0 ? (L.next || 'Next') : (L.save || 'Save')) + '</button>' +
            '</div>' +
        '</div>';		 

    if (pdfAtts.length > 0) {
        const cb = document.getElementById('emlPdfBodyLastCb');
        const listEl = document.getElementById('emlPdfReorderList');
        const radios = document.querySelectorAll('input[name="emlSaveFmt"]');
        const wrap = document.getElementById('emlPdfOrderWrap');
        
        const toggleWrap = () => {
            wrap.style.display = (document.querySelector('input[name="emlSaveFmt"]:checked').value === 'pdf') ? 'block' : 'none';
        };
        toggleWrap();
        radios.forEach(r => r.addEventListener('change', toggleWrap));

        const renderReorderList = () => {
            listEl.innerHTML = '<div style="font-size:11px; color:var(--text-secondary); margin-bottom:4px; font-weight:bold; text-transform:uppercase;">' + (L.pdf_drag_reorder_seq || 'Drag to reorder sequence:') + '</div>';
            pdfAtts.forEach((att, idx) => {
                const item = document.createElement('div');
                item.draggable = true;
                item.style.cssText = 'padding:6px 10px; background:var(--gray-00); border:1px solid var(--border-medium); border-radius:4px; font-size:12px; cursor:grab; display:flex; align-items:center; gap:8px; transition:border-color 0.1s;';
                item.innerHTML = '<span style="color:var(--text-secondary); cursor:grab; font-weight:bold;">≡</span> <span style="overflow:hidden; text-overflow:ellipsis; white-space:nowrap; flex:1;">' + myCloudEscapeHtml(att.filename || att.name) + '</span>' + 
                                 '<span class="ce-remove-att-btn" style="cursor:pointer; color:var(--danger); padding:0 4px; font-weight:bold; font-size:14px;" title="' + (L.pdf_exclude_att || 'Exclude from PDF') + '">✕</span>';
                
                item.querySelector('.ce-remove-att-btn').onclick = (e) => {
                    e.stopPropagation();
                    excludedPdfAtts.push(att.filename || att.name);
                    pdfAtts.splice(idx, 1);
                    renderReorderList();
                };
                item.ondragstart = (e) => { e.dataTransfer.setData('text/plain', idx); item.style.opacity = '0.5'; };
                item.ondragend = () => { item.style.opacity = '1'; };
                item.ondragover = (e) => { e.preventDefault(); item.style.borderTop = '2px solid var(--accent-primary)'; };
                item.ondragleave = () => { item.style.borderTop = '1px solid var(--border-medium)'; };
                item.ondrop = (e) => {
                    e.preventDefault(); item.style.borderTop = '1px solid var(--border-medium)';
                    const fromIdx = parseInt(e.dataTransfer.getData('text/plain'));
                    if (fromIdx !== idx && !isNaN(fromIdx)) {
                        const moved = pdfAtts.splice(fromIdx, 1)[0];
                        pdfAtts.splice(idx, 0, moved);
                        renderReorderList();
                    }
                };
            ...
            bodyItem.style.cssText = 'padding:6px 10px; background:var(--hover-bg-light); border:1px dashed var(--border-medium); border-radius:4px; font-size:12px; display:flex; align-items:center; gap:8px; color:var(--text-secondary); margin-top:4px;';
            bodyItem.innerHTML = '<span>✉️</span> <span style="font-style:italic; flex:1;">' + (L.template_body || 'Email Body') + '</span>';
            listEl.appendChild(bodyItem);
        };
        cb.onchange = () => {
            if (cb.checked && pdfAtts.length > 1) { listEl.style.display = 'flex'; renderReorderList(); } 
    ...
    document.getElementById('btnEmlDoSave').onclick = async () => {
        ...
        const pdfBodyLast = document.getElementById('emlPdfBodyLastCb') && document.getElementById('emlPdfBodyLastCb').checked;
        const pdfOrder = pdfBodyLast ? pdfAtts.map(a => a.filename || a.name) : [];
		const pdfExclude = pdfBodyLast ? excludedPdfAtts : [];
        ...
        if (target === 'local') {
            ...
            for (let i = 0; i < targetKeys.length; i++) {
               ...
               try {
                    let dlSuccess = false;
                    if (fmt === 'pdf') {
                        dlSuccess = await window._emailDownloadPdf(aId, fld, mId, forceFallback, pdfBodyLast, pdfOrder, pdfExclude);
                    } else {
            ...
            window._emailSaveToCloudBulk(targetKeys, fmt, writeableClouds, delAfter, pdfBodyLast, pdfOrder, pdfExclude);
        }
    };
};
...
window._emailShowCloudTreeModal = function(options) {
    ...
    modal.innerHTML = `
        ...
        <div class="myCloudModalBody" style="display:flex; flex-direction:column; flex:1; overflow:hidden;">
            ...
            <div id="ceEmlTreeFavsContainer" style="display:flex; flex-direction:column; margin-bottom:10px; border:1px solid var(--border-medium); border-radius:4px; overflow:hidden; flex-shrink:0;">
                <div style="display:flex; justify-content:space-between; align-items:center; background:var(--gray-10); padding:4px 8px; border-bottom:1px solid var(--border-medium);">
                    <span style="font-size:11px; font-weight:bold; color:var(--text-secondary); text-transform:uppercase;">⭐ ${L.fav_title || 'Favorites'}</span>
                    <button id="ceEmlAddFavFolderBtn" class="cx-btn" style="padding:2px 6px; font-size:11px; height:20px; line-height:1;">➕ ${L.folder || 'Folder'}</button>
                </div>
                <style>.ce-fav-drop-before { border-top-color: var(--accent-primary) !important; } .ce-fav-drop-after { border-bottom-color: var(--accent-primary) !important; } .ce-fav-drop-inside { background-color: var(--hover-bg-medium) !important; border-color: var(--accent-primary) !important; }</style>
                <div id="ceEmlTreeFavsWrap" style="padding:4px; min-height:36px; max-height:150px; overflow-y:auto; background:var(--gray-00);"></div>
            </div>
    ...
    document.getElementById('ceEmlAddFavFolderBtn').onclick = () => {
        treeFavs.unshift({ id: 'fav_' + Math.random().toString(36).substr(2,9), type: 'folder', name: 'New Group', isOpen: true, children: [] });
        saveFavs();
    };

    const checkIsFav = (path) => {
        let found = false;
        const checkList = (list) => {
            for (let i=0; i<list.length; i++) {
                if (list[i].type === 'link' && list[i].cloud === currentCloud && list[i].path === path) found = true;
                if (list[i].type === 'folder' && list[i].children) checkList(list[i].children);
            }
        };
        checkList(treeFavs);
        return found;
    ...
    const findParentAndIndex = (id, list) => {
        for (let i = 0; i < list.length; i++) {
            if (list[i].id === id) return { parent: list, index: i };
            if (list[i].type === 'folder' && list[i].children) {
                const res = findParentAndIndex(id, list[i].children);
                if (res) return res;
            }
        }
        return null;
    ...
    const removeFavNode = (id) => {
        const loc = findParentAndIndex(id, treeFavs);
        if (loc) return loc.parent.splice(loc.index, 1)[0];
        return null;
    };

    const insertFavNode = (node, targetId, position) => {
        if (targetId === 'root') { treeFavs.push(node); return; }
        const loc = findParentAndIndex(targetId, treeFavs);
        if (loc) {
            if (position === 'inside' && loc.parent[loc.index].type === 'folder') {
                if (!loc.parent[loc.index].children) loc.parent[loc.index].children = [];
                loc.parent[loc.index].children.push(node);
                loc.parent[loc.index].isOpen = true;
            } else if (position === 'before') {
                loc.parent.splice(loc.index, 0, node);
            } else if (position === 'after') {
                loc.parent.splice(loc.index + 1, 0, node);
            }
        } else { treeFavs.push(node); }
    };
    ...
    const fetchFavs = async () => {
        ...
        if (res.status === 'OK') { 
            const loadedFavs = res.favorites || []; 
            treeFavs = loadedFavs.map(f => {
                if (f.type) return f;
                return { id: 'fav_' + Math.random().toString(36).substr(2,9), type: 'link', cloud: f.cloud, path: f.path, name: f.name };
            });
            renderFavs(); 
        }
    };
    ...
    const toggleFav = (path, name) => {
        ...
        const findLink = (list) => {
            for (let i=0; i<list.length; i++) {
                if (list[i].type === 'link' && list[i].cloud === currentCloud && list[i].path === path) return list[i].id;
                if (list[i].type === 'folder' && list[i].children) {
                    const id = findLink(list[i].children);
                    if (id) return id;
                }
            }
          return null;
        };
        existingId = findLink(treeFavs);

        if (existingId) {
            removeFavNode(existingId);
        } else {
            treeFavs.push({ id: 'fav_' + Math.random().toString(36).substr(2,9), type: 'link', cloud: currentCloud, path: path, name: name });
        }
        saveFavs();
    };

    const renderFavTreeUI = (list, container, level = 0) => {
        list.forEach(node => {
            const wrap = document.createElement('div');
            const el = document.createElement('div');
            el.className = 'ce-fav-node';
            el.style.cssText = `display:flex; align-items:center; gap:6px; padding:4px 6px; padding-inline-start:${level * 16 + 6}px; border:1px solid transparent; border-radius:3px; cursor:pointer; font-size:12px; color:var(--text-primary); transition:background 0.1s;`;
            el.draggable = true;

            el.onmouseover = () => { el.style.background = 'var(--gray-10)'; el.querySelector('.ce-fav-actions').style.display = 'flex'; };
            el.onmouseout = () => { el.style.background = 'transparent'; el.querySelector('.ce-fav-actions').style.display = 'none'; };

            el.ondragstart = (e) => { e.stopPropagation(); e.dataTransfer.setData('text/plain', node.id); e.dataTransfer.effectAllowed = 'move'; el.style.opacity = '0.5'; };
            el.ondragend = () => { el.style.opacity = '1'; document.querySelectorAll('.ce-fav-drop-before, .ce-fav-drop-after, .ce-fav-drop-inside').forEach(x => x.classList.remove('ce-fav-drop-before', 'ce-fav-drop-after', 'ce-fav-drop-inside')); };
            el.ondragover = (e) => {
                e.preventDefault(); e.stopPropagation();
                const rect = el.getBoundingClientRect();
                const relY = e.clientY - rect.top;
                el.classList.remove('ce-fav-drop-before', 'ce-fav-drop-after', 'ce-fav-drop-inside');
                if (node.type === 'folder') {
                    if (relY < rect.height * 0.25) el.classList.add('ce-fav-drop-before');
                    else if (relY > rect.height * 0.75) el.classList.add('ce-fav-drop-after');
                    else el.classList.add('ce-fav-drop-inside');
                } else {
                    if (relY < rect.height * 0.5) el.classList.add('ce-fav-drop-before');
                    else el.classList.add('ce-fav-drop-after');
                }
            };
            el.ondragleave = () => el.classList.remove('ce-fav-drop-before', 'ce-fav-drop-after', 'ce-fav-drop-inside');
            el.ondrop = (e) => {
                e.preventDefault(); e.stopPropagation();
                el.classList.remove('ce-fav-drop-before', 'ce-fav-drop-after', 'ce-fav-drop-inside');
                ...
                if (node.type === 'folder') {
                    let isDesc = false;
                    const checkDesc = (n) => { if (n.id === node.id) isDesc = true; if (n.type === 'folder' && n.children) n.children.forEach(checkDesc); };
                    const loc = findParentAndIndex(draggedId, treeFavs);
                    const dNode = loc ? loc.parent[loc.index] : null;
                    if (dNode && dNode.type === 'folder' && dNode.children) { dNode.children.forEach(checkDesc); if (dNode.id === node.id) isDesc = true; if (isDesc) return; }
                }
                ...
                let pos = 'after';
                if (node.type === 'folder') {
                    if (relY < rect.height * 0.25) pos = 'before';
                    else if (relY < rect.height * 0.75) pos = 'inside';
                } else {
                    if (relY < rect.height * 0.5) pos = 'before';
                }
                const dn = removeFavNode(draggedId);
                if (dn) { insertFavNode(dn, node.id, pos); saveFavs(); }
            };

            const iconWrap = document.createElement('div');
            iconWrap.style.cssText = 'display:flex; align-items:center; justify-content:flex-start; width:34px; height:18px; flex-shrink:0; user-select:none; font-size:13px;';
			if (node.type === 'folder') {
                iconWrap.innerHTML = (node.isOpen ? '<span style="width:14px; display:inline-block; text-align:center;">▾</span>' : '<span style="width:14px; display:inline-block; text-align:center;">▸</span>') + '<span style="display:inline-flex; align-items:center;">' + (typeof myCloudIconFolder !== 'undefined' ? myCloudIconFolder : '📁') + '</span>';
				iconWrap.style.cursor = 'pointer';
                iconWrap.onclick = (e) => { e.stopPropagation(); node.isOpen = !node.isOpen; saveFavs(); };
            } else { iconWrap.innerHTML = '<span style="width:14px; display:inline-block;"></span><span style="color:#f0ad4e; font-size:14px; display:inline-flex; align-items:center;">★</span>'; }
            ...
            if (node.type === 'link') {
                nameTxt.onclick = (e) => {
                    e.stopPropagation();
                    if (!availableClouds.includes(node.cloud)) { if (typeof cxToast === 'function') cxToast('Cloud unavailable', false); return; }
                    cloudSel.value = node.cloud; currentCloud = node.cloud; currentPath = node.path;
                    selectedItem = mode === 'folder' ? node.path : null;
                    expandedPaths = new Set(['/']);
                    let walker = ''; node.path.split('/').filter(p => p).forEach(part => { walker += '/' + part; expandedPaths.add(walker); });
                    btnOk.disabled = true; loadRoot();
                };
            } else {
                nameTxt.onclick = (e) => { e.stopPropagation(); node.isOpen = !node.isOpen; saveFavs(); }
            }

            const inputTxt = document.createElement('input');
            inputTxt.type = 'text'; inputTxt.className = 'myCloudInlineInput'; inputTxt.value = node.name;
            inputTxt.style.cssText = 'display:none; width:100%; height:20px; font-size:12px; margin:0; padding:0 4px;';
            nameWrap.appendChild(inputTxt);

            const actWrap = document.createElement('div');
            actWrap.className = 'ce-fav-actions';
            actWrap.style.cssText = 'display:none; align-items:center; gap:8px; flex-shrink:0;';

            const editBtn = document.createElement('span'); editBtn.innerHTML = '✎'; editBtn.style.cssText = 'cursor:pointer; opacity:0.6;';
            editBtn.onclick = (e) => { e.stopPropagation(); nameTxt.style.display = 'none'; inputTxt.style.display = 'block'; inputTxt.focus(); };
            const delBtn = document.createElement('span'); delBtn.innerHTML = '✕'; delBtn.style.cssText = 'cursor:pointer; color:var(--danger); opacity:0.8;';
            delBtn.onclick = (e) => { e.stopPropagation(); if (confirm('Delete ' + (node.type==='folder'?'group':'favorite') + '?')) { removeFavNode(node.id); saveFavs(); } };

            inputTxt.onblur = () => { node.name = inputTxt.value.trim() || node.name; saveFavs(); };
            inputTxt.onkeydown = (e) => { if (e.key === 'Enter') inputTxt.blur(); if (e.key === 'Escape') { inputTxt.value = node.name; inputTxt.blur(); } };

            actWrap.appendChild(editBtn); actWrap.appendChild(delBtn);
            el.appendChild(iconWrap); el.appendChild(nameWrap); el.appendChild(actWrap);
            wrap.appendChild(el);

            if (node.type === 'folder' && node.isOpen && node.children) {
                const childContainer = document.createElement('div');
                childContainer.style.width = '100%';
                renderFavTreeUI(node.children, childContainer, level + 1);
    ...
    const renderFavs = () => {
        ...
        if (treeFavs.length === 0) {
            wrap.innerHTML = `<div style="color:var(--text-secondary); font-size:12px; font-style:italic; padding:10px; text-align:center;">${L.fav_folders_empty || 'Favorite folders will appear here...'}</div>`;
        } else {
			renderFavTreeUI(treeFavs, wrap, 0);
        }
        wrap.ondragover = (e) => { e.preventDefault(); };
        wrap.ondrop = (e) => {
            e.preventDefault(); const draggedId = e.dataTransfer.getData('text/plain');
            if (draggedId && e.target === wrap) { const dNode = removeFavNode(draggedId); if (dNode) { insertFavNode(dNode, 'root', 'after'); saveFavs(); } }
        };
    ...
    const renderTree = async (container, path) => {
        ...
        items.forEach(item => {
            ...
            let favBtnHtml = '';
            if (isDir) {
                const isFav = checkIsFav(fullPath);
...
window._emailSaveToCloudBulk = function(targetKeys, format, availableClouds, delAfter = false, pdfBodyLast = false, pdfOrder = [], pdfExclude = []) {
    const L = typeof myCloud_LANG !== 'undefined' ? myCloud_LANG : {};
        ...
        onConfirm: async (currentCloud, currentPath) => {
            ...
            for (let i = 0; i < targetKeys.length; i++) {
                ...
                try {
                    ...
                    if (format === 'pdf') {
                        fd.append('load_images', loadImg);
                        if (pdfBodyLast) { fd.append('pdf_body_last', '1'); fd.append('pdf_attachment_order', JSON.stringify(pdfOrder)); fd.append('pdf_excluded_atts', JSON.stringify(pdfExclude)); }
                    }
